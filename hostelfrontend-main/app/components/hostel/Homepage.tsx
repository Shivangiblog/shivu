...
//           }>(`http://localhost:5000/api/hostels/${hostel._id}/complaints`, {
//             headers: { Authorization: `Bearer ${token}` },
//           });
...
const HostelCard: React.FC<HostelCardProps> = ({ hostel }) => (
  <Card className="bg-white text-gray-800 shadow-md hover:shadow-lg transition-all duration-300 rounded-lg overflow-hidden mb-2">
...
// const FeesCard: React.FC<FeesCardProps> = ({ label, amount, color }) => (
//   <Card
//     className={`bg-gradient-to-r ${color} text-white shadow-md hover:shadow-lg transition-all duration-300 rounded-lg h-full`}
//   >
...
export default function Dashboard() {
  // Keep all the useState and useEffect hooks as they were
  const [ownerFormOpen, setOwnerFormOpen] = useState<boolean>(false);
  const [hostels, setHostels] = useState<HostelCardProps["hostel"][]>([]);
  const [stats, setStats] = useState<{
    totalHostels: number;
  ...
  return (
    ...
    >
      <Typography
        variant="h4"
        gutterBottom
        className="text-gray-900 font-extrabold mb-4 text-center"
      >
        Dashboard
      </Typography>
      <Grid container spacing={2}>
        <Grid item xs={12} md={9}>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={6}>
              <Card className="bg-white text-gray-800 shadow-md rounded-lg p-4 h-full">
                <CardContent>
                  <Typography
                    variant="h6"
                    className="font-bold text-gray-900 mb-4"
        ...
        <Grid item xs={12} md={3}>
          <Card className="bg-white shadow-md rounded-lg p-2 h-full">
            <CardContent>
